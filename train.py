...
for epoch in range(epoch_num):
    ...
    for i,(inputs,labels) in enumerate(train_loader,0):
        ...
        epoch_loss += loss.item() * inputs.size(0)  # 累加 batch 的损失
        if(i+1)%num_print==0:
            print('[%d epoch,%d]  loss:%.6f' %(epoch+1,i+1,running_loss/num_print))
            running_loss=0.0

    epoch_loss /= len(train_loader.dataset)  # 计算平均训练损失
    ...
    with torch.no_grad():  
        for inputs, labels in test_loader:  
            ...
            val_loss += loss.item() * inputs.size(0)  # 累加 batch 的损失
        
            _, preds = torch.max(outputs, 1)  
            val_corrects += torch.sum(preds == labels.data)  
            val_total += labels.size(0)  
        
    val_loss /= val_total  # 计算平均验证损失
    val_acc = val_corrects.double() / val_total  

    #wandb.log({"val_loss": val_loss, "val_acc": val_acc})  # 同时记录验证损失和准确率
    ...
    if schedule:  
        schedule.step(val_loss)  # 如果 scheduler 需要验证损失作为输入

    print(f'Epoch {epoch+1}/{epoch_num} - Loss: {epoch_loss:.4f} - Val Loss: {val_loss:.4f} - Val Acc: {val_acc:.4f}')
